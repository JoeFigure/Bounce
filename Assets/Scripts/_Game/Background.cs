...
public class Background : MonoBehaviour {
	...
	public GameObject gradient, planets, movingSprites, mountains;
	...
	void Start () {
		planetStartPos = new Vector2 (-(GameplayController.screenWidth/2), GameplayController.screenHeight/2);
		mountainStartPos = new Vector2 (planetStartPos.x, planetStartPos.y - 4);

		PositionGradient ();
		CreateNewBackground ();
		planets.transform.position = planetStartPos;
		mountains.transform.position = mountainStartPos;
	...
	void CreateNewBackground(){
		...
		planets.transform.position = new Vector2(GameplayController.screenWidth, planetStartPos.y);
		planets.transform.SetParent(movingSprites.transform);

		mountains = Instantiate (mountains) as GameObject;
		mountains.transform.position = new Vector2(planets.transform.position.x, mountainStartPos.y);
